...
if($sessn){
	...
	//$strSQL = "SELECT * FROM esub_mst WHERE (SESSION LIKE '%".$sessn."%')";
        //echo $_POST["SEM_CD"];
        if(isset($_GET["SEM_CD"]) && $_GET["SEM_CD"])
            $semcd=$_GET["SEM_CD"];
        else
            $semcd=$_POST["SEM_CD"];
	...
	$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
        ...
        echo "<br>".$num." records are listed below.......";
	?>
...
fpassthru($fp);
	...
	<br>
        <TABLE><TR><TD>
                    <form action="index_course.php" method="post">
...
<form action="downloadxls.php?SEM_CD=<?php echo $semcd; ?>&SESSION=<?php echo $sessn;?>&xls=<?php echo "y";?>" method="post">
<input type="submit" value="Take this in excel" />
</form>
